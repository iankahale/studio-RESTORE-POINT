...
import { updateAuctionItem } from '@/lib/data';
import { AuctionItem, auctionCategories, AuctionItemCategory } from '@/lib/types';
import { Textarea } from './ui/textarea';
import Image from 'next/image';
import { Upload, X, Wand2, Loader2, CheckCircle } from 'lucide-react';
...
type EditAuctionItemFormValues = z.infer<typeof auctionItemSchema>;

interface EditAuctionItemDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    onItemUpdated: (item: AuctionItem) => void;
    item: AuctionItem;
}
...
export function EditAuctionItemDialog({ open, onOpenChange, onItemUpdated, item }: EditAuctionItemDialogProps) {
    ...
    const form = useForm<EditAuctionItemFormValues>({
    ...
    const onSubmit = (data: EditAuctionItemFormValues) => {
        startTransition(async () => {
            try {
                let statusUpdate: { status?: 'Listed' } = {};
                let toastMessage = "Auction item has been updated.";

                if (item.status === 'Draft') {
                    statusUpdate.status = 'Listed';
                    toastMessage = "Item has been published and is now live.";
                }

                const updatedItem = await updateAuctionItem(item.id, { ...data, ...statusUpdate });
                ...
                toast({ variant: 'destructive', title: "Error", description: "Failed to update auction item." });
            }
        });
    };
    ...
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-2xl grid-rows-[auto_1fr_auto] max-h-[90svh]">
        <DialogHeader>
          ...
          <DialogDescription>Update the details for the auction item below. You can add or remove images.</DialogDescription>
        </DialogHeader>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-6 py-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormField control={form.control} name="imageUrls" render={({ field }) => (
                            <FormItem className="md:col-span-2">
                                <FormLabel>Product Images (max 3MB each)</FormLabel>
                                <FormControl>
                                    <div>
                                        <Input id="file-upload-edit" type="file" multiple accept="image/jpeg,image/png,image/gif,image/webp" onChange={handleFileChange} className="hidden" ref={fileInputRef} disabled={isUploading}/>
                                        <label htmlFor="file-upload-edit" className={`flex items-center justify-center w-full h-32 px-4 transition bg-background border-2 border-dashed rounded-md appearance-none ${isUploading ? 'cursor-not-allowed' : 'cursor-pointer hover:border-primary focus:outline-none'}`}>
                                            <span className="flex items-center space-x-2">
                                                <Upload className="h-6 w-6 text-muted-foreground" />
                                                <span className="font-medium text-muted-foreground">
                                                    {isUploading ? 'Processing images...' : 'Click to upload more files'}
                                                </span>
                                            </span>
                                        </label>
                                    </div>
                    ...
                    <DialogFooter className="mt-4 sticky bottom-0 bg-background z-10 py-4 -mx-6 px-6">
                        ...
                        <Button type="submit" disabled={isPending || isUploading}>
                            {isPending ? "Saving..." : (
                                isDraft ? <> <CheckCircle className="mr-2 h-4 w-4" /> Save & Publish </> : "Save Changes"
                            )}
                        </Button>
                    </DialogFooter>
                </form>
